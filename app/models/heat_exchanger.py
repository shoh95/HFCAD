...
def heat_exchanger_model(power_stacks, volt_cell, cell_temp, mu_f, v_cr, m_cr, p_cr, t_cr, rho_cr, mu_cr):
    ...
    # return 1.2*(m_al + 2*m_h2o), [(np.ceil(np.sqrt(n_tube))+1)*h_pass, (np.ceil(np.sqrt(n_tube))+1)*h_pass, l_tube]
    return q_all, 1.2*(m_al + 2*m_h2o), [(np.ceil(np.sqrt(n_tube))+1)*h_pass, (np.ceil(np.sqrt(n_tube))+1)*h_pass, l_tube]
