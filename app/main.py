...
def size_system(power_fc_sys, volt_req, h_cr, mach_cr, oversizing, beta, comp_bool, n_stacks_series):
    ...
    # print(f"\nStack(s): {m_stacks:,.0f} kg, Compressor: {m_comp:,.0f} kg, Humidifier: {m_humid:,.0f} kg, Heat Exchanger: {m_hx:,.0f} kg, mdot_h2: {mdot_h2:,.2f} kg/s, Pcomp: {power_comp/1000:,.3f} kW")
...
if __name__ == "__main__":
    ...
    print(f"\nStack(s): {sizing_results['m_stacks']:,.0f} kg, Compressor: {sizing_results['m_comp']:,.0f} kg, Humidifier: {sizing_results['m_humid']:,.0f} kg, Heat Exchanger: {sizing_results['m_hx']:,.0f} kg, mdot_h2: {sizing_results['mdot_h2']:,.2f} kg/s), Pcomp: {sizing_results['power_comp']/1000:,.3f} kW, eta_fcsys: {sizing_results['eta_fcsys']:,.3f}")
    
    figs = sizing_results['figs']
    figs[-1].show()
