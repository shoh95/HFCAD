...
class FuelCellSizing:
    def __init__(self):
        ...
        # self.Mission_time = [5, 15, 16, 23, 25, 70, 71, 100, 110, 112, 116, 118, 135, 135, 135, 140, 150, 160, 160]
        ...
        self.eta_FC = 0.5               ##SH: FCS 모듈로 스택단위 효율 계산으로 변경, 이 값은 초기값임
        ...
        self.P_W_climb = 0.1139*1000/9.81 ###0.1139*1000/9.81 #14.89 #15.97  # Power to Weight case1(climb),2(cruise),3(descent)
        self.P_W_cruise = 0.0876*1000/9.81 ###0.0876*1000/9.81 #10.53
        self.P_W_takeoff = 0.0739*1000/9.81 ###0.0739*1000/9.81 #8.6
        ...
        #Pcooling
        self.Tair = Atmosphere(self.h_cr).temperature[0] # 268.67 # 3000 m
        self.Tt1 = self.Tair * (1 + 0.4 / 2 * self.mach_cr ** 2)  # total temperature at cruise in K # 268.67 # 3000 m
        self.dT = 60
        ...
        self.q_cruise_imp = self.q_cruise * (0.0208855)         #SH: psf
        ...
        self.Dfus_imp = Conversions.meter_feet(self, 1.85, 'ft')         # 1.85 * 3.281 #ft
        ...
        # self.eta_fuelcell = 0.6 #연료전지 효율은 가정 ##SH: 사용하지 않음..
    ...
    def cal_Ptotal_climb(self, MTOM_trial, PFC1=5000000):
        ...
        while True:
            ...
            pemfcsys_sizing_results_climb = FuelCellSystem.size_system(power_fc_sys_parallel, self.volt_req, self.h_cr, self.mach_cr, self.oversizing, self.beta, comp_bool, self.n_stacks_series, make_fig=False, verbose=False)
            ...
            if abs(self.ptotal_b - self.ptotal_a) <= 100:
                ...
                # self.P_W_climb = Pelectricnet1 / (MTOM_trial*9.81)
    ...
    def cal_Ptotal_cruise(self, MTOM_trial, PFC2=5000000):
        ...
        while True:
            ...
            pemfcsys_sizing_results_cruise = FuelCellSystem.size_system(power_fc_sys=power_fc_sys_parallel, volt_req=self.volt_req, h_cr=self.h_cr, mach_cr=self.mach_cr, oversizing=self.oversizing, beta=self.beta, comp_bool=comp_bool, n_stacks_series=self.n_stacks_series, make_fig=False, verbose=False)
            ...
            if abs(self.ptotal_d - self.ptotal_c) <= 100:
                ...
                # Pshaft2 = MTOM_trial * 9.81 * self.P_W_cruise
                # Pshaft2 = self.ptotal_d
                # MTOM_trial = self.Pfuelcell_cruise / (9.81 * self.P_W_cruise) --
                # self.P_W_cruise = self.ptotal_d / (MTOM_trial*9.81)
    ...
    def cal_Ptotal_cruise_charger(self, MTOM_trial, PFC3=5000000):
        ...
        while True:
            ...
            pemfcsys_sizing_results_cruise_charger = FuelCellSystem.size_system(power_fc_sys_parallel, self.volt_req, self.h_cr, self.mach_cr, self.oversizing, self.beta, comp_bool, self.n_stacks_series, make_fig=False, verbose=False)
            # mair3 = pemfcsys_sizing_results_cruise_charger['m_dot_air'] * self.n_stacks_parallel             #SH: per Nacelle
            # Calculate fuel cell power system power including compressor
            # self.Pcomp3 = mair3 * self.Cp_air * (self.Tt2-self.Tt1) / self.eta_em  # Placeholder value for compressor power
            self.Pcomp3 = pemfcsys_sizing_results_cruise_charger['power_comp'] * self.n_stacks_parallel      #SH: per Nacelle
    ...
    def cal_Ptotal_takeoff(self, MTOM_trial, PFC4=500000):
        ...
        while True:
            ...
            # power_fc_sys_parallel = (self.ptotal_g)/self.n_stacks_parallel
            comp_bool = True

            pemfcsys_sizing_results_takeoff = FuelCellSystem.size_system(power_fc_sys=power_fc_sys_parallel, volt_req=self.volt_req, h_cr=0., mach_cr=0.16, oversizing=self.oversizing, beta=1.05, comp_bool=comp_bool, n_stacks_series=self.n_stacks_series, make_fig=False, verbose=False)
    ...
    def cal_MTOM(self, MTOM_trial):
        ...
        comp_bool = True
        pemfcsys_sizing_results = FuelCellSystem.size_system(power_fc_sys_parallel, self.volt_req, self.h_cr, self.mach_cr, self.oversizing, self.beta, comp_bool, self.n_stacks_series, make_fig=False, verbose=False)
        ...
        # self.Pcomp3 = self.n_stacks_parallel*pemfcsys_sizing_results["power_comp"]

        #PMAD and electric motor power

        # self.Pheat_rejected1 = self.n_stacks_parallel*pemfcsys_sizing_results['q_all']/1000
        ...
        # # self.Mass_fuel_old = self.n_stacks_parallel*pemfcsys_sizing_results['mdot_h2']*160*60          # (mdot_H2 [kg/s] X 160*60 [s])
...
class FuelCellSystem:
    ...
    def singlecell(self):
        ...
        # # print(self.self.beta)
        # eta_0 = np.arcsinh( (self.j_0 / self.j_sigma)**2 / ( 2*(self.c_h / self.c_ref)*( 1 - np.exp(-1*self.j_0/(2*self.jstar)) ) ) ) + self.sigma_t * self.b_tfslope**2 / (4*self.F_const*self.D*self.c_h) * ( self.j_0/self.jstar - np.log( 1 + (self.j_0**2/(self.jstar**2 * self.self.beta**2)) ) ) * ( 1- self.j_0/(self.jstar_lim * self.c_h/self.c_ref) )**(-1) - self.b_tfslope * np.log( 1- self.j_0 / (self.jstar_lim * self.c_h/self.c_ref) )
    ...
    def size_system(
        ...
        *,
        ...
        Parameters match the legacy implementation, with additional keyword-only controls:

        - make_fig: when True, return a Plotly polarization curve figure (expensive).
                    Keep this False during iterative sizing loops.
        - verbose: enable per-call console output (costly in large loops).
        - comp_tol_w: absolute convergence tolerance for compressor-power fixed-point iteration (W).
        - max_comp_iter: safety cap on compressor-power iteration count.

        Performance notes (practical):
        - The cell polarization scan is power-independent; the upstream model now caches it.
        - Plot generation is the dominant overhead if enabled repeatedly.
        """
        ...
        c_cr = atm_cr.speed_of_sound[0]  # speed of sound at altitude in m/s
        v_cr = mach_cr * c_cr            # true airspeed in m/s
        p_cr = atm_cr.pressure[0]        # static pressure in Pa
        p_cr_tot = p_cr * (1 + 0.4 / 2 * mach_cr ** 2) ** (1.4 / 0.4)  # total pressure in Pa
        t_cr = atm_cr.temperature[0]     # static temperature in K
        t_cr_tot = t_cr * (1 + 0.4 / 2 * mach_cr ** 2)                # total temperature in K
        rho_cr = atm_cr.density[0]       # density in kg/m3
        mu_cr = atm_cr.dynamic_viscosity[0]  # dynamic viscosity in Pa s
        ...
        pres_cathode_in = beta * p_cr_tot if comp_bool else p_cr_tot

        # Cell model (cached + vectorized in models.stack_functions)
        pres_h = Atmosphere(0).pressure[0]  # anode inlet pressure ~ sea level
        ...
        # Compressor models
        if comp_bool:
            # Fixed-point iteration: power_req = power_fc_sys + Pcomp(power_req)
            power_req_new = float(power_fc_sys)
            power_comp = 0.0
            geom_comp = None
            rho_humid_in = rho_cr
            m_dot_comp = None
            ...
            for _ in range(int(max_comp_iter)):
                ...
                power_req_new = float(power_fc_sys) + float(power_comp)
                if abs(power_req_new - power_req) <= tol:
                    break
            else:
                # Non-fatal: proceed with last iterate
                if verbose:
                    print(
                        f"WARNING: compressor iteration hit max_comp_iter={max_comp_iter} "
                        f"(|ΔP|={abs(power_req_new - power_req):.3f} W, tol={tol:.3f} W)."
                    )

            m_comp = compressor_mass_model(geom_comp, power_comp) if geom_comp is not None else 0.0
        else:
            # no compressor
            m_comp = 0.0
            power_comp = 0.0
...
if __name__ == "__main__":
    ...
    # Generate PEMFC polarization figure once (expensive; avoid inside iterative loops)
    try:
        pemfcsys_sizing_results_fig = FuelCellSystem.size_system(
            power_fc_sys_parallel,
            FCS.volt_req,
            FCS.h_cr,
            FCS.mach_cr,
            FCS.oversizing,
        ...
        figs = pemfcsys_sizing_results_fig.get('figs', [])
        if figs:
            fig = figs[-1]
            # fig.show()  # optional
        ...
        print(f"WARNING: Could not generate PEMFC figure: {e}")
